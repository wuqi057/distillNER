...
from data_KD import Label #NOTE: change for KD
...
class SequenceTagger(flair.nn.Model):
    ...
    def evaluate(
            ...
            verbose: bool = False,#NOTE: added function
    ) -> (Result, float):
    ...
    def _calculate_loss(
        ...
        #NOTE: change for KD
        tag_list_dist: List[List[float]] = []
        for s_id, sentence in enumerate(sentences):
            # get the tags in this sentence
            tag_idx_dist: List[int] = [
                [d.score for d in token.get_tags_proba_dist(self.tag_type)]
                for token in sentence
            ]
            # add tags as tensor
        ...
        if self.KD: #NOTE: change for KD
            score = 0
            for sentence_feats_dist, sentence_tags_dist, sentence_length in zip(
                    features, tag_list_dist, lengths
            ):
                sentence_feats = sentence_feats[:sentence_length]
            ...
            if get_all_tags:
                all_tags.append(
                    [
                        [
                            Label(
                                self.tag_dictionary.get_item_for_index(score_id), np.float16(score)# NOTE
                            )
